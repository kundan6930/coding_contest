...
/*--------------------------------------------------------------------------------------------------------------------------*/ 
ll n;
ll solve1(vl &a,ll pos,ll l, ll r,ll f1,ll f2)
{
    if(pos==n)
    {
            if(l==0 && r==n-1)
    ...
    cout<<pos<<" "<<l<<" "<<r<<endl;
    ll mini=1000;
    mini=min(mini,solve1(a,pos+1,max({l,(ll)0,pos-a[pos]+1}),r,1,(r!=-1)?1:0)+1);
    mini=min(mini,solve1(a,pos+1,l,min({r,n-1,pos+a[pos]-1}),(l!=n)?1:0,1)+1);
    mini=min(mini,solve1(a,pos+1,l,r,(l!=n)?1:0,(r!=-1)?1:0));
    return mini;
}
void solve() {
   int t;
