...
vl factorial(ll n,ll mod){vl fact(n+1);fact[0]=1;fl(i,1,n,1){fact[i]=fact[i-1]*i; fact[i]%=mod;} return fact; }
...
void solve() {
 ...
 //  cout<<o<<" "<<z<<" "<<endl;
         ...
         {
              vl fact=factorial(m+1,Mod); 
              ll p=o+z;
              ll ans=fact[p];
              ans%=Mod;
              ans*=mminvprime(fact[o],Mod);
              ans%=Mod;
              ans*=mminvprime(fact[z],Mod);
              ans%=Mod;
              cout<<ans<<endl;
         }
   }
