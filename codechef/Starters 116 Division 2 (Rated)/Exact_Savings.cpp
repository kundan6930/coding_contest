...
using ll=long long;
   ll n,x,z;
   ll solve1(vl &a,ll pos, ll mod1, vvl &dp)
   {
       if(pos==n)
       {
          if(mod1==0)
       ...
       ll mini=1e9;
        mini=min(mini,solve1(a,pos+1,(mod1+a[pos])%x,dp))+(mod1+a[pos])/x;
        mini=min(mini,solve1(a,pos+1,mod1,dp));
       return dp[pos][mod1]=mini;
   }
void solve() {
   int t;
   cin>>t;
